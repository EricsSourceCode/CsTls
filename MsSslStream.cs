...
// This is still just the example code from
// Microsoft.
...
// Make a Tls.cs class so you can use TLS
// without having to know the implementation.
// So it doesn't need to use the
// MsSslStream class.
...
public class MsSslStream
{
// static X509Certificate serverCertificate = null;

// The certificate parameter specifies the
...
while (true)
  ...
  // Application blocks while waiting for
...
sslStream.Write( message );
  ...
  {
  // The client stream will be closed
  // with the sslStream because we specified
...
// The client signals the end of the
// message using the "<EOF>" marker.
...
Console.WriteLine(
            ...
            stream.CanRead, 
            stream.CanWrite);
Console.WriteLine( "Can timeout: {0}",
                        stream.CanTimeout);
}
*/
...
if( stream.LocalCertificate != null )
  {
  Console.WriteLine(
 "Local cert was issued to {0} and
 is valid from {1} until {2}.",
...
if( stream.RemoteCertificate != null )
  {
  Console.WriteLine(
         "Remote cert was issued to {0} and
      is valid from {1} until {2}.",
